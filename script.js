...
// index.html의 <script src=".../supabase-js@2"> 주석도 함께 해제해야 합니다.
...
function buildCertFields(cert) {
    ...
    if (cert === "공인중개사") {
        ...
        const firstPassBox = document.createElement("div");
        firstPassBox.id = "firstPassBox";
        firstPassBox.className = "step--hidden";
        firstPassBox.innerHTML = `
      <div class="field">
        <label class="checkbox-row">
          <input type="checkbox" id="isFirstPassHolder"> 1차 합격자입니다
        </label>
      </div>
      <div class="field-pair">
        <div class="field">
          <label for="firstPassYear">1차 합격연도</label>
          <input type="number" id="firstPassYear" placeholder="예) 2026" min="2000" max="2100">
        </div>
        <div class="field">
          <label for="firstPassNumber">1차 합격번호</label>
          <input type="text" id="firstPassNumber" placeholder="예) 2026-12345">
        </div>
      </div>
    `;
        container.appendChild(firstPassBox);
        ...
        stageSelect.addEventListener("change", () => {
            ...
            firstPassBox.classList.toggle("step--hidden", stage !== "2차");
...
applyForm.addEventListener("submit", async (e) => {
    ...
    const payload = {
        ...
        is_first_pass_holder: document.getElementById("isFirstPassHolder")?.checked ?? null,
        first_pass_year: val("firstPassYear") ? Number(val("firstPassYear")) : null,
        first_pass_number: val("firstPassNumber"),
